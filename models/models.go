...
var (
	dbuser string = "root"
	dbpwd  string = "BASIC123+"
	dbname string = "beelog"
)

//初始化orm
func RegisterDB() {
	conn := dbuser + ":" + dbpwd + "@/" + dbname + "?charset=utf8&loc=Local" //组合成连接串
	orm.RegisterModel(new(Category), new(Topic), new(Reply))                 // 注册 Category 如果没有会自动创建
